...
class TripletWithBSRnn(LightningModule):
    ...
    def __init__(
        ...
        self.featurizer = torchaudio.transforms.Spectrogram(
            n_fft=cfg.f_size,
            win_length=cfg.f_size,
            hop_length=cfg.hop_length,
            power=None
        )
        self.inverse_featurizer = torchaudio.transforms.InverseSpectrogram(
            n_fft=cfg.f_size,
            win_length=cfg.f_size,
            hop_length=cfg.hop_length,
        )
    ...
    def get_loss_unet_bsrnn(
        ...
        for idx, c in enumerate(triposi):
            lossR = self.loss_unet(
                predS[idx].real, tgtS[idx, 0].real
            )
            lossI = self.loss_unet(
                predS[idx].imag, tgtS[idx, 0].imag
            )
            # time domain
            lossT = self.loss_unet(
                predT[idx], tgtT[idx, 0]
            )
            loss_dict = {
                "lossSpecR": lossR,
                "lossSpecI": lossI,
    ...
    def get_loss_triplet(self, e_a, e_p, e_n, triposi):
        ...
        #dist_p_all = {inst: torch.sum(distp[torch.where(triposi==i)])/len(torch.where(triposi==i)[0]) for i,inst in enumerate(self.cfg.inst_list)}
        #dist_n_all = {inst: torch.sum(distn[torch.where(triposi==i)])/len(torch.where(triposi==i)[0]) for i,inst in enumerate(self.cfg.inst_list)}
        loss_inst = {inst: torch.sum(loss)/len(triposi) for inst in self.cfg.inst_list}
        dist_p_all = {inst: torch.sum(distp)/len(triposi) for inst in self.cfg.inst_list}
        dist_n_all = {inst: torch.sum(distn)/len(triposi) for inst in self.cfg.inst_list}
        return loss_all, loss_inst, dist_p_all, dist_n_all
    ...
    def forward(self, batch):
        ...
        # stft
        a_x = self.featurizer(a_x_wave); a_y = self.featurizer(a_y_wave)
        p_x = self.featurizer(p_x_wave); p_y = self.featurizer(p_y_wave)
        n_x = self.featurizer(n_x_wave); n_y = self.featurizer(n_y_wave)
        # forward
        a_e, a_prob, a_pred = self.net(a_x)
        p_e, p_prob, p_pred = self.net(p_x)
        n_e, n_prob, n_pred = self.net(n_x)
        # istft
        a_pred_t = self.inverse_featurizer(a_pred)
        p_pred_t = self.inverse_featurizer(p_pred)
        n_pred_t = self.inverse_featurizer(n_pred)
        # get loss
        loss_unet_a = self.get_loss_unet_bsrnn(a_pred, a_y, a_pred_t, a_y_wave, triposi)
        loss_unet_p = self.get_loss_unet_bsrnn(p_pred, p_y, p_pred_t, p_y_wave, triposi)
        loss_unet_n = self.get_loss_unet_bsrnn(n_pred, n_y, n_pred_t, n_y_wave, triposi)
    ...
    def model_step_psd(self, mode:str, batch, idx):
        ...
        with torch.no_grad():
            data = self.featurizer(data_wave)
        ...
        self.recorder_psd[mode]["vec"][self.cfg.inst_list[idx]].append(embvec)
    ...
    def test_step(self, batch, batch_idx: int, dataloader_idx=0) -> None:
        ...
        if dataloader_idx == 0:
            ...
            cases_x = self.featurizer(cases_x)
            ...
            if not self.cfg.mel:
                for idx,c in enumerate(cases):
                    if ((c[0] == 1 and c[1] == 1 and c[2] == 1 and c[3] == 1 and c[4] == 1)
                        ...
                        sound = self.inverse_featurizer(cases_x[idx])
                        ...
                        for j,inst in enumerate(self.cfg.inst_list):
                            ...
                            sound = self.featurizer(c_pred[idx, 0])
