...
from ..to1d.model_embedding import EmbeddingNet128to128, To1dEmbedding, EmbeddingNet640to640
...
class UNetForTriplet_to1d640_1dde1_embnet(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq"):
        ...
        # To2d from 1d
        self.to2d = To2DFrom1D640(out_channel=(f_size/2/(2**6)+1)*encoder_out_size)
        # embeddint net
        self.embnet = EmbeddingNet640to640()
        #self.tanh = nn.Tanh()
    ...
    def forward(self, input):
        ...
        csn1d = ConditionalSimNet1d(size[0]) # csnのモデルを保存されないようにするために配列に入れる
        # to1d
        output_emb = self.to1d(conv6_out)
        # to2d
        decoder_in = self.to2d(output_emb, size[3])
        # Decoder
        output_decoder_ch = self.decoder(decoder_in, conv5_out, conv4_out, conv3_out, conv2_out, conv1_out, input)
        # embnet
        output_embnet = self.embnet(output_emb)

        for idx, inst in enumerate(self.inst_list):
            # decoder
            output_decoder[inst] = torch.unsqueeze(output_decoder_ch[:,idx], dim=1)
            masked_emb = csn1d(output_embnet, torch.tensor([idx], device=device))  # 特徴量を条件づけ
            # probability
            output_probability[inst] = torch.log(torch.sqrt(torch.sum(masked_emb**2, dim=1))) # logit
        return output_embnet, output_probability, output_decoder
