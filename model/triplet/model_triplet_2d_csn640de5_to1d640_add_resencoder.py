...
class UNetResEncoder(nn.Module):
    def __init__(self, encoder_in_size, encoder_out_size):
        ...
        #deviceを指定
        self.to(device)
    def forward(self, input, conv1_out, conv2_out, conv3_out, conv4_out, conv5_out, conv6_out):
        # Encoder
        conv1_outr = self.conv1(input)
        conv2_outr = self.conv2(conv1_outr + conv1_out)
        conv3_outr = self.conv3(conv2_outr + conv2_out)
        conv4_outr = self.conv4(conv3_outr + conv3_out)
        conv5_outr = self.conv5(conv4_outr + conv4_out)
        conv6_outr = self.conv6(conv5_outr + conv5_out)
        return conv6_outr + conv6_out
...
class UNetForTriplet_2d_de5_to1d640_addresencoder(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", mel=False, n_mels=259):
        ...
        #self.linear = nn.Linear(640*2, 640)
        #self.tanh = nn.Tanh()
...
class UNetForTriplet_2d_de5_to1d640_addencoder2(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", mel=False, n_mels=259):
        super().__init__()
        if mono:
            encoder_in_size = 1
        ...
        #self.to1d = To1D640(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*encoder_out_size)
        if mel:
            in_channel = (n_mels//(2**6)+1)*encoder_out_size * 2
        else:
            in_channel = (f_size/2/(2**6)+1)*encoder_out_size * 2
        self.to1d = To1D640(to1d_mode=to1d_mode, order=order, in_channel=in_channel)
        #self.tanh = nn.Tanh()
        self.sigmoid = nn.Sigmoid()
        #deviceを指定
