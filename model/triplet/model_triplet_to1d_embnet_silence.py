...
from ..csn import ConditionalSimNet2d768
...
class UNetDecoder(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.deconv6_a = nn.ConvTranspose2d(768, 256, kernel_size = (5, 5), stride=(2, 2), padding=2)
...
class To1dEmbedding(nn.Module):
    def __init__(self, to1d_mode, order, in_channel, tanh = True) -> None:
        ...
        self.to(device)
    def forward(self, input):
        return self.embnet(input)

class UNetForTriplet_2d_de5_embnet_silence(nn.Module):
    def __init__(self, inst_list, f_size, to1d_mode="mean_linear", order="timefreq", tanh = True):
        super().__init__()
        # Encoder
        ...
        self.decoder_silence= UNetDecoder()
        # Embedding Network
    ...
    def forward(self, input):
        ...
        if not "csn" in vars(self):
            self.csn = ConditionalSimNet2d768(size, device)
        embnet = {
        ...
        for idx, inst in enumerate(self.inst_list):
            ...
            # embedding
            if inst != "silence":
                # 分離音のエンコーダ出力(分離音+mix音特徴量)から無音エンコーダ出力(mix音特徴量)を引く（分離音特徴だけが残るはず）
                emb = embnet[inst](conv6_out[:,128*idx : 128*(idx+1), :,:] - conv6_out[:,640 : 768, :,:])
                output_emb[inst] = emb
...
def main():
    ...
    model = UNetForTriplet_2d_de5_embnet_silence(inst_list=inst_list, f_size=1024)
    batchsize = 16
    summary(model=model,
