...
class UNetDecoder(nn.Module):
    def __init__(self) -> None:
        ...
        self.deconv5_b = nn.Sequential(
                                ...
                                nn.Dropout2d(0.5))
        """
        self.deconv4_a = nn.ConvTranspose2d(128, 64, kernel_size = (5, 5), stride=(2, 2), padding=2)
        ...
        #deviceを指定
        self.to(device)
    def forward(self, conv4_out, conv3_out, conv2_out, conv1_out, input):
        ...
        deconv4_out = self.deconv4_a(conv4_out, output_size = conv3_out.size())
...
class UNetForTripletInst(nn.Module):
    def __init__(self):
        super().__init__()
        # Encoder
    ...
    def forward(self, input):
        ...
        #decoder_out = self.decoder(conv4_out, conv3_out, conv2_out, conv1_out, input)
        return conv4_out
    
if "__main__" == __name__:
    # モデルを定義
    model = UNetForTripletInst()
    batchsize = 16
    summary(model=model,
