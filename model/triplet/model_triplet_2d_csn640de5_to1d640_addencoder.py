...
class UNetForTriplet_2d_de5_to1d640_addencoder(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", mel=False, n_mels=259):
        ...
        self.addto1d = To1D640(to1d_mode=to1d_mode, order=order, in_channel=in_channel)
    ...
    def forward(self, input):
        ...
        # to1d
        emb = self.to1d(conv6_out)
        addemb = self.addto1d(conv6_out)
        output_emb = self.linear(torch.concat([emb, addemb], dim=1))
