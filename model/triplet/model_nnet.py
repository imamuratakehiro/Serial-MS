...
class UNetDecoder(nn.Module):
    ...
    def forward(self, sep_feature, conv5_out, conv4_out, conv3_out, conv2_out, conv1_out, input):
        ...
        output = deconv1_out
        return output, deconv2_out, deconv3_out, deconv4_out, deconv5_out, deconv6_out
...
class UNetEncoder2(nn.Module):
    def __init__(self, encoder_in_size, encoder_out_size):
        ...
        # Encoder
        self.conv1 = Conv2d(encoder_in_size, 16)
        self.conv2 = Conv2d(32, 32)
        self.conv3 = Conv2d(64, 64)
        self.conv4 = Conv2d(128, 128)
        self.conv5 = Conv2d(256, 256)
        self.conv6 = Conv2d(512, encoder_out_size, last=True)
        #deviceを指定
        self.to(device)
    def forward(self, input, deconv2_out, deconv3_out, deconv4_out, deconv5_out, deconv6_out):
        # Encoder
        conv1_out = self.conv1(input)
        conv2_out = self.conv2(torch.cat([conv1_out, deconv2_out], dim=1))
        conv3_out = self.conv3(torch.cat([conv2_out, deconv3_out], dim=1))
        conv4_out = self.conv4(torch.cat([conv3_out, deconv4_out], dim=1))
        conv5_out = self.conv5(torch.cat([conv4_out, deconv5_out], dim=1))
        conv6_out = self.conv6(torch.cat([conv5_out, deconv6_out], dim=1))
        return conv6_out
...
class NNetForTriplet(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", mel=False, n_mels=259):
        ...
        decoder_out_size = len(inst_list)
        encoder2_in_size = len(inst_list)*encoder_in_size# + 1
        # Encoder
        self.encoder = UNetEncoder(encoder_in_size, encoder_out_size)
        #self.encoder2 = UNetEncoder2(encoder2_in_size, encoder_out_size)
        self.encoder2 = UNetEncoder(encoder2_in_size, encoder_out_size)
        # Decoder
        self.decoder = UNetDecoder(decoder_out_size, encoder_out_size)
        """
    ...
    def forward(self, input):
        ...
        """
        output_de, deconv2_out, deconv3_out, deconv4_out, deconv5_out, deconv6_out = self.decoder(
            conv6_out, conv5_out, conv4_out, conv3_out, conv2_out, conv1_out, input
        )
        #encoder2_in = torch.concat([output_de, input], dim=1)
        encoder2_in = output_de
        #output_encoder2 = self.encoder2(encoder2_in, deconv2_out, deconv3_out, deconv4_out, deconv5_out, deconv6_out)
        _, _, _, _, _, output_encoder2 = self.encoder2(encoder2_in)
        ...
        """
        # to1d
        output_emb = self.to1d(output_encoder2)
