...
class TripletWithTFCTDFUNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.to1d = To1D640(to1d_mode=config.to1d_mode, order=config.order, in_channel=f*c)
    ...
    def forward(self, x):
        ...
        x = x.transpose(-1,-2) # 時間次元と周波数次元を入れ替えた
...
def main(cfg):
    ...
    model = TripletWithTFCTDFUNet(cfg.train)
    batchsize = 16
    summary(model=model,
            input_size=(batchsize, 1, 10*44100),
