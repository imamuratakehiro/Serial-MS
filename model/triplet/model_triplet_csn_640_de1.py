"""Tripletのためのモデル。UNet部分は出力640次元を128次元で条件付けしてDecoderに入力。Decoderは1つ"""
...
class UNetForTriplet640De1(nn.Module):
    ...
    def forward(self, input):
        ...
        for idx, inst in enumerate(self.inst_list):
            ...
            decoder_out = self.decoder(sep_feature, conv5_out, conv4_out, conv3_out, conv2_out, conv1_out, input)
            output[inst] = decoder_out
        return conv6_out, output
    
if "__main__" == __name__:
    # モデルを定義
    model = UNetForTriplet640De1()
    batchsize = 16
    summary(model=model,
