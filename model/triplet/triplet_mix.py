...
class Triplet_mix(LightningModule):
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        # update and log metrics
        self.val_loss(loss_triplet_all)
        self.log("val/loss_all", self.val_loss, on_step=True, on_epoch=True, prog_bar=False)
        for inst in self.cfg.inst_list:
            self.val_loss_triplet[inst] = loss_triplet[inst]
            self.val_dist_p[inst] = dist_p[inst]
            self.val_dist_n[inst] = dist_n[inst]
            self.log(f"val/loss_{inst}", self.val_loss_triplet[inst], on_step=True, on_epoch=True, prog_bar=True)
            self.log(f"val/dist_p_{inst}", self.val_dist_p[inst], on_step=True, on_epoch=True, prog_bar=True)
            self.log(f"val/dist_n_{inst}", self.val_dist_n[inst], on_step=True, on_epoch=True, prog_bar=True)
    ...
    def test_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        self.test_emb.append(e[self.cfg.inst])
    ...
    def on_test_epoch_end(self) -> None:
        ...
        vec   = torch.concat(self.test_emb, dim=0).to("cpu").numpy()
