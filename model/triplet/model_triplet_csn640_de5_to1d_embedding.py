...
class UNetForTriplet_2d_de5_embnet(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq", tanh = True):
        ...
        for inst in inst_list:
            if inst == "drums":
                ...
                self.embnet_drums  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
            elif inst == "bass":
                self.decoder_bass = UNetDecoder(encoder_in_size, encoder_out_size)
                self.embnet_bass  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
            elif inst == "piano":
                self.decoder_piano = UNetDecoder(encoder_in_size, encoder_out_size)
                self.embnet_piano  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
            elif inst == "guitar":
                self.decoder_guitar = UNetDecoder(encoder_in_size, encoder_out_size)
                self.embnet_guitar  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
            elif inst == "vocals":
                self.decoder_vocals = UNetDecoder(encoder_in_size, encoder_out_size)
                self.embnet_vocals  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
            elif inst == "residuals":
                self.decoder_residuals = UNetDecoder(encoder_in_size, encoder_out_size)
                self.embnet_residuals  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
            elif inst == "other":
                self.decoder_other = UNetDecoder(encoder_in_size, encoder_out_size)
                self.embnet_other  = To1dEmbedding(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*128)
        #self.tanh = nn.Tanh()
