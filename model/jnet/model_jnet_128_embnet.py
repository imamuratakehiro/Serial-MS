...
class JNet128Embnet(nn.Module):
    def __init__(self, inst_list, f_size, mono=True, to1d_mode="mean_linear", order="timefreq"):
        super().__init__()
        if mono:
            encoder_in_size = 1
        else:
        ...
        self.encoder_out_size = 4 * 128
        # Encoder
        self.encoder = UNetEncoder(encoder_in_size, self.encoder_out_size)
        # Decoder・Embedding Network
        for inst in inst_list:
            if inst == "drums":
                self.embnet_drums  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
            elif inst == "bass":
                self.embnet_bass  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
            elif inst == "piano":
                self.embnet_piano  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
            elif inst == "guitar":
                self.embnet_guitar  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
            elif inst == "vocals":
                self.embnet_vocals  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
            elif inst == "residuals":
                self.embnet_residuals  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
            elif inst == "other":
                self.embnet_other  = To1D128(to1d_mode=to1d_mode, order=order, in_channel=(f_size/2/(2**6)+1)*self.encoder_out_size)
        #self.tanh = nn.Tanh()
    ...
    def forward(self, input):
        ...
        output_emb = {}
        output_probability = {}
        #output_emb = torch.zeros(B, self.encoder_out_size)
